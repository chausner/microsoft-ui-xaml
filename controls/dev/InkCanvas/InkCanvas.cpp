...
void InkCanvas::AttachToVisualLink()
{
    ...
    // Fork on the compositor engine: a system-backed process splices the ink visual under a lifted
    // MUC visual; a lifted process bridges it into the XAML tree via ContentExternalOutputLink. Each
    // path binds the ink visual to the presenter (AttachInkVisualToPresenter) first, then roots it
